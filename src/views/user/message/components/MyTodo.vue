...
<script lang="ts">
  ...
  import { getMyTodoPageAPI } from '@/api/common/login-api';
  ...
  export default defineComponent({
    ...
    setup(props, { emit, slots, attrs }) {
      ...
      const reload = (pageParam: PageParam) => {
        ...
        getMyTodoPageAPI(queryParam).then((data) => {
          emit('updateUnReadNum', 'todo', data['total']);
          tableRef.value?.setDataSource(data['list'] || [], pageParam.current, data['total']);
        }).catch((e) => {
      ...
      return { tableRef, columns, reload, handleQuery, handleShow };
    },
    methods: {},
  });
</script>
