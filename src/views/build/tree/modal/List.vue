  ...
  <div :class="['xin-page-full']">
    ...
    <a-card style="margin-top: 12px" :body-style="{ height: '100%', padding: '16px 16px 10px 16px' }">
      <x-pro-table
        ...
        <template #toolbarR>
          <div style="margin-left: 5px; font-size: 14px;">树窗口编辑<b>懒加载</b>生成实例，大数据量请使用<b>懒加载</b></div>
        </template>
...
export default defineComponent({
  ...
  setup(props, {emit, slots}) {
    ...
    const dataSource = (pageParam: PageParam) => {
      ...
      return httpRequest('/tree/modal/list', 'post', queryParam);
    };
    ...
    const handleOk = (newData) => {
      if (newData.pid == 0) {
        ...
        tableRef.value?.refreshNode(newData.pid);
      }
    };
