...
<script lang="ts">
import {ruleCheck, formUtil, rowId, transformDictData, transformTreeList} from "xin-antd3-ui";
...
export default defineComponent({
  ...
  setup(props, {emit, slots}) {
    ...
    const parseData = (list) => {
      //接口'/tree/modal/all/list'，需要再转换成树结构列表
      const treeList = transformTreeList(list, 0, 'idleaf', 'pid');
      return transformDictData(treeList, {value: 'idleaf', label: 'name'});
    };
    ...
    const handleLoadForm = (data?, parentRow?) => {
      handleReset();
    ...
    return {title, visible, formLoading, submitLoading, isQuery, formRef, form, rules, parseData, handleSubmit, handleReset, handleClose, openDialog};
  },
  methods: {},
});
</script>
