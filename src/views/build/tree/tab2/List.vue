  ...
  <div :class="['xin-page-full']">
    ...
    <a-card style="margin-top: 12px" :body-style="{ height: '100%', padding: '16px 16px 10px 16px' }">
      <x-pro-table
        ...
        <template #toolbarR>
          <div style="margin-left: 5px; font-size: 14px;">树选项卡编辑<b>全部加载</b>生成实例，数据量小请使用<b>全部加载</b></div>
        </template>
...
export default defineComponent({
  ...
  setup(props, {emit, slots}) {
    ...
    const dataSource = (pageParam: PageParam) => {
      ...
      return httpRequest('/tree/tab/all/list', 'post', queryParam);
    };
    ...
    const handleShow = (e, record) => {
      stopPropagation(e);
      signPushTabPage(router, {path: '/build/tree/tab/all/form', query: {type: 'query', id: record.idleaf}});
    };

    const handleAdd = (e, parentRow) => {
      stopPropagation(e); //阻止冒泡事件
      signPushTabPage(router, {path: '/build/tree/tab/all/form', query: {type: 'add', pid: parentRow?.idleaf ?? 0}});
    };

    const handleEdit = (e, record) => {
      stopPropagation(e);
      signPushTabPage(router, {path: '/build/tree/tab/all/form', query: {type: 'edit', id: record.idleaf}});
    };
