  ...
  <div :class="['xin-page-full']">
    ...
    <a-card style="margin-top: 12px" :body-style="{ height: '100%', padding: '16px 16px 10px 16px' }">
      <x-pro-table
        ...
        <template #toolbarR>
          <span style="margin-left: 5px; font-size: 14px;">树行编辑<b>全部加载</b>生成实例，数据量小请使用<b>全部加载</b></span>
        </template>
        <template #contextmenuPopup="{ args }">
          <a-menu :selectable="false" style="width:100px;">
            <a-menu-item :disabled="addDisabled(record)" @click="(e) => handleAdd(e, args.record, args)">
...
export default defineComponent({
  ...
  setup(props, {emit, slots}) {
    ...
    const dataSource = (pageParam: PageParam) => {
      ...
      return httpRequest('/tree/row/all/list', 'post', queryParam);
    };
