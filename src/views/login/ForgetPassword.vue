<template>
  <div class="login-form">
    <h2>重置密码<span class="t2">演示版v1.0.0</span></h2>
    <div class="v-tag">Vue3.x版本</div>
    <div style="margin: 10px 0px 10px 0px" @keypress="loginPress">
      <a-form :model="userInfo">
        <a-form-item name="username">
          <a-input allow-clear size="large" v-model:value="userInfo.username" placeholder="请输入登录账号">
            <template #prefix>
    ...
    <div class="loging-btn">
      <a-button size="large" style="width: 100%; margin-top: 0px" :loading="loading" type="primary" @click="handleSubmit">
        <span v-if="!loading">重置密码</span>
        <span v-else>正在登录...</span>
      </a-button>

      <a-button size="large" style="width: 100%; margin: 20px 0px 10px 0px" :loading="loading" @click="changeForm('login')"> 返回</a-button>
    </div>
  </div>
</template>
